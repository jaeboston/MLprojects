...
def create_ranked_df(movies, reviews):
        ...
        #: rename the column 'date' to 'last_rating'
        last_rating.columns = ['last_rating']

        # Add Dates
        #: join() when both rating_count_df and last_rating have same index, movie_id: 
        # rating_count_df.join(last_rating), index column is intact when join() is used
        rating_count_df = pd.DataFrame({'avg_rating': avg_ratings, 'num_ratings': num_ratings})
        rating_count_df = rating_count_df.join(last_rating)
        #: you can use merge() instead of join as shown below
        #rating_count_df = rating_count_df.merge(last_rating, left_on='movie_id', right_on='movie_id'
...
if __name__ == '__main__':
    ...
    #: check the create_ranked_df function
    movies = pd.read_csv('RecommenderSystems/MovieRecommender/movies_clean.csv')  
    reviews = pd.read_csv('RecommenderSystems/MovieRecommender/train_data.csv')
